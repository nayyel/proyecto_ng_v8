...
export class LibrosComponent implements OnInit { // Agregado OnInit
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      this.MostrarLibro(libroId); // Corrección aquí
    });
  }
//Metodo de mostrar el libro cuando ya le das click
      ...
      for (let i = 0; i < this.libros.length; i++) {
        if (this.libros[i].id_libro == libroId) {
          ...
          <div class='libro' id='${this.libros[i].id_libro}'>
            <img src='${this.libros[i].link}'>
            <h2 id="Titulo">${this.libros[i].titulo}</h2>
            <p id="Autor">Autor: ${this.libros[i].autor}</p>
            <p id="Resumen">Resumen: toda tu ${this.libros[i].resumen}</p>
